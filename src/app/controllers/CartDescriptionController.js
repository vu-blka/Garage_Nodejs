...
class CartDescriptionController {
    ...
    getById(req, res, next) {
        ...
        Cart.findOneWithDeleted({ cartId: id }).then((cart) => {
            if (cart) {
    ...
    add(req, res, next) {
        ...
        if (JSON.stringify(data) === '{}') {
            ...
            CartDescription.findOne({ cartId: data?.idCartDes }).then(
                (cart) => {
                    if (cart) {
                        ...
                        cart.save()
                            .then(() => {
                                res.status(200).send(
                                    'Thêm sản phẩm (add) vào giỏ hàng thành công'
                                );
                            })
                            .catch((error) => {
                                console.log(error);
                        ...
                        res.status(401).send(
                            'Không tìm thấy cart description tương ứng với ID này'
                        );
                    }
                }
            );
                        ...
                        .then(() => {
                            Cart.findOneAndUpdate(
                                ...
                                .then(() =>
                                    res
                                        .status(200)
                                        .send('Đã chấp nhận yêu cầu của garage')
                                )
                                .catch((error) => res.status(401).send(error));
                        })
                        .catch((error) => res.status(401).send(error));
            ...
            CartDescription.findOneAndUpdate({ _id: id }, { productAdd: [] })
                .then(() => {
                    Cart.findOne({ _id: id })
                        .then((cart) => {
                            res.status(200).send(
                                'Xóa sản phẩm thêm bởi garage thành công'
                            );
                        })
                        .catch((error) => res.status(401).send(error));
