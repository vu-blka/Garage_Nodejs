...
class UserController {
  ...
  async update(req, res, next) {
    ...
    if (JSON.stringify(data) === '{}')
      res
        .status(401)
        .send('Thiếu thông tin thay đổi');
    else {
	  const user = await User.findOne({idCardNumber: data?.idCardNumber}).exec();
	  if(user){
		User.updateOne({idCardNumber: data?.idCardNumber}, data)
		.then(() => res.status(200).send('Đổi thông tin thành công!'))
		.catch(error => res.status(401).send(error))
	  }
	  else
	  {
	...
	else
	{
		...
		if(account)
		{
			...
			if(result)
			{
				...
				account.save()
					.then(() => res.status(200).send('Đổi mật khẩu thành công'))
					.catch(error => res.status(401).send(error))
			}
			else
			{
